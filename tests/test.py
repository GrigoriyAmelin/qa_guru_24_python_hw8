...
def test_send_single_email_two():
    ...
    assert len(result) == 1
    assert result[0].status.value == Status.FAILED.value
...
def test_email_address_valid():
    ...
    assert addr.email_address == "user@gmail.com"
...
def test_send_email_single_recipient():
    ...
    assert sent.recipients[-1].email_address == "b@b.com"
...
def test_send_email_multiple_recipients():
    ...
    assert {msg.recipients[0].email_address for msg in results} == {"b@b.com", "c@c.com", "d@d.com"}
...
def test_recipients_auto_wraps_to_list():
    ...
    assert isinstance(email.recipients, list)
    assert len(email.recipients) == 1
    assert email.recipients[0].email_address == "b@b.com"
...
def test_send_email_single_recipient_creates_new_object():
    ...
    assert sent.recipients[0].email_address == "b@b.com"
...
def test_email_address_valid_variants(valid):
    assert EmailAddress(valid).email_address == valid.lower().strip()
